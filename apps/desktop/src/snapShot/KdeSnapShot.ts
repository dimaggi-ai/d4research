...
import type { DesktopCaptureHelperState } from "@d4research/contracts";
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall d4research.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall d4research.",
        );
