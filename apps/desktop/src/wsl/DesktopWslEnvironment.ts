...
import { buildRemoteNodeEnvScript } from "@d4research/ssh/tunnel";
...
>()("@d4research/desktop/wsl/DesktopWslEnvironment") {}
...
export const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_PREBUILD_MISSING_EXIT_CODE
    ? "WSL support is missing from this d4research build: the packaged Linux node-pty binary was not included. Rebuild the Windows artifact with `--wsl-prebuild <path-to-linux-pty.node>` or install a build that includes WSL support."
    : null;
