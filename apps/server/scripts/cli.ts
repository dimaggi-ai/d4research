...
const buildExeCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      yield* Effect.log(
        "[cli] Built dist-exe/d4research (expects client/, resource-monitor/, and the runtime-external node_modules beside it; scripts/build-cli-archive.ts assembles that tree)",
      );
    }),
).pipe(
 ...
 * every `@d4research/d4research-<platform>.tgz` first, `d4research.tgz` (the launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@d4research");
      const launcherTarball = path.join(packagesDir, "d4research.tgz");
      const platformTarballs = (yield* fs
        .readDirectory(scopeDir)
        .pipe(Effect.orElseSucceed((): ReadonlyArray<string> => [])))
        .filter((entry) => entry.startsWith("d4research-") && entry.endsWith(".tgz"))
        .sort()
        .map((entry) => path.join(scopeDir, entry));
      if (platformTarballs.length === 0) {
        return yield* new ServerCliBuildAssetMissingError({
          assetPath: path.join(scopeDir, "d4research-<platform>.tgz"),
        });
      }
  ...
  Command.withDescription(
    "Publish the @d4research/d4research-<platform> tarballs and then the d4research launcher to npm.",
  ),
);
