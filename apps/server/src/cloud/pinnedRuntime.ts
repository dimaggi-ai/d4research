...
/**
 * A pinned runtime is an exact `d4research@<version>` npm-installed into
 ...
 * switching over, never `npx d4research`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "d4research", "dist", "bin.mjs"),
...
/**
 * Installs `d4research@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "d4research", "dist", "bin.mjs"),
