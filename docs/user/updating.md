...
`npx d4research@<version> service update` command on the server machine. That one local update installs the
...
**Copy update command** gives you `npx d4research@<client-version>`, which relaunches the server directly
...
```sh
npx d4research@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `npx d4research@latest service update`
...
3. For a command-line server, relaunch it with `npx d4research@<client-version>`, replacing
