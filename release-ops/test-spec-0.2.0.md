...
| P1-2 | No turn remains indefinitely "running"/"starting" after its provider works or dies — codex path | pump lifetime fix (`CodexAdapter.ts` fork into session scope) with a fails-against-old regression test; real-CLI handoff matrix (`providerHandoffMatrix.real.integration.test.ts`, opt-in); the reopened MCP leg is now exercised end-to-end by `mcpDelegateTool.real.integration.test.ts` (opt-in, `T3_REAL_MCP=1`): a real codex session calls `memory_status` over a live harness-served `/mcp` and the turn quiesces | automated (opt-in)                  |
| P1-3 | Same guarantee for the remaining providers' event pipelines                                     | pump-lifetime audit 2026-08-15: Claude (`runFork` root fiber), OpenCode and ACP runtime (Grok/Junie) fork into session/runtime scope, Agy awaits process exit in-scope — all safe; Cursor had the same `forkChild` bug as codex, fixed with a fails-against-old regression test in `CursorAdapter.test.ts`                                                                                                               | automated                           |
