...
describe("cliRelease", () => {
  it("names archives by version and platform, zip only on Windows", () => {
    expect(cliArchiveFileName("1.2.3-preview.20260911.4", "linux-x64")).toBe(
      "d4research-1.2.3-preview.20260911.4-linux-x64.tar.gz",
    );
    expect(cliArchiveFileName("1.2.3", "win32-x64")).toBe("d4research-1.2.3-win32-x64.zip");
  });
  ...
  it("resolves download URLs under the tagged release, honoring a mirror", () => {
    expect(cliReleaseDownloadBaseUrl("1.2.3")).toBe(
      "https://github.com/dimaggi-ai/d4research/releases/download/v1.2.3",
    );
  ...
  it("parses sha256sum output including binary-mode markers", () => {
    const checksums = parseChecksums(
      [
        `${"a".repeat(64)}  d4research-1.2.3-linux-x64.tar.gz`,
        `${"B".repeat(64)} *d4research-1.2.3-win32-x64.zip`,
        "not a checksum line",
        "",
      ].join("\n"),
    );
    expect(checksums.get("d4research-1.2.3-linux-x64.tar.gz")).toBe("a".repeat(64));
    expect(checksums.get("d4research-1.2.3-win32-x64.zip")).toBe("b".repeat(64));
  ...
  it("pages through the release index at the largest page GitHub allows", () => {
    expect(cliReleaseIndexPageUrl(1)).toBe(
      "https://api.github.com/repos/dimaggi-ai/d4research/releases?per_page=100&page=1",
    );
