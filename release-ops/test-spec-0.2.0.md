...
| P1-2 | No turn remains indefinitely "running"/"starting" after its provider works or dies — codex path | poison-handler isolation: `packages/effect-codex-app-server/src/client.test.ts`; pump hardening in `CodexSessionRuntime.ts`/`CodexAdapter.ts` (2026-08-14 checkpoint) | automated                           |
