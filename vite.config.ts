...
const RESTRICTED_IMPORT_PATHS = [
  {
    name: "@d4research/client-runtime",
    message:
      "Import from an explicit @d4research/client-runtime/* subpath. The package has no root export.",
  },
  {
  ...
  lint: {
    ...
    overrides: [
      ...
      },
      {
        ...
        excludeFiles: ["apps/web/src/components/ui/**"],
        rules: {
          // d4research: warn, not error. Fork-only UI still restyles primitives; upstream's
          // ceiling script was removed, so an error here would fail every web lint.
          "shadcn/no-restyle": [
            "warn",
            {
              allow: ["layout"],
