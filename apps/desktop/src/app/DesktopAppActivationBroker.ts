...
export class DesktopAppActivationBroker {
  ...
  request(request: DesktopAppActivationRequest): Promise<DesktopAppActivationResponse> {
    if (this.#closed) {
      return Promise.resolve(
        failure(request.requestId, "renderer-unavailable", "d4research is shutting down."),
      );
    }
      ...
      if (pending.dispatched) {
        this.#settle(
          failure(
            ...
            "The d4research window closed before it opened the project.",
          ),
        );
      }
    }
  ...
  cancel(requestId: string): void {
    this.#settle(
      failure(requestId, "renderer-unavailable", "The command closed before d4research was ready."),
    );
  }
  ...
  close(): void {
    ...
    for (const pending of this.#pending.values()) {
      this.#settle(
        failure(pending.request.requestId, "renderer-unavailable", "d4research is shutting down."),
      );
    }
  }
