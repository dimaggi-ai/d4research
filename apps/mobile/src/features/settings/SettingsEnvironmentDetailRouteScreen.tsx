...
function EnvironmentDetail({ environmentId }: { readonly environmentId: EnvironmentId }) {
  ...
  function requestServerUpdate() {
    ...
    Alert.alert(
      ...
      `Install d4research ${targetVersion}. ${capabilities.serverSelfUpdate === "desktop-managed" ? "The desktop app will close and relaunch." : "The server will restart and reconnect."} Running threads may be interrupted.`,
      [
      ...
      >
        {!environment ? (
          ...
          <>
            ...
            {config ? (
              <>
                <SettingsSection title="d4research">
                    ...
                    {!supportsEnvironmentUpdate(config.environment.capabilities) ? (
                      <Text className="text-sm text-foreground-muted">
                        {capabilities?.serverSelfUpdate === "desktop-managed"
                          ...
                          : "Update and restart d4research on this machine."}
                      </Text>
                    ) : null}
                  </View>
                  <SettingsActionRow
