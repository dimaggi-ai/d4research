...
it("does not activate T3 until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "d4research" },
    { ...window, id: 2, pid: process.pid, title: "Other T3" },
    { ...window, id: 3, pid: process.pid, title: "d4research" },
  ];
  await snapshot.feedback!.activate("d4research");
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: {
          window: { ...window, id: 4, pid: process.pid, title: "d4research" },
        },
      });
  };
  await snapshot.feedback!.activate("d4research");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "d4research" }));
  await expect(snapshot.feedback!.activate("d4research")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("d4research")).rejects.toThrow("cancelled");
