...
The production macOS bundle ID is `ai.dimaggi.d4research`. To enable native passkeys:

1. Create an explicit macOS App ID for `ai.dimaggi.d4research` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.ai.dimaggi.d4research`.
