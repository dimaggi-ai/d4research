...
import type {
  ThreadSettingsDevPipelines,
  ThreadSettingsSessionSkill,
...
type ThreadSettingsSessionProps = {
  ...
  /**
   * Research/dev pipeline session controls. Optional: not every caller wires
   * these up yet, so the sheet renders without them when absent instead of
   * requiring every consumer to pass placeholders.
   */
...
function ThreadSettingsSessionProvider(
  ...
  const value = useMemo<ThreadSettingsSessionValue>(
    () => ({
      ...
      pipelineTargetPolicy: props.pipelineTargetPolicy,
      onUpdatePipelineTargetPolicy: props.onUpdatePipelineTargetPolicy,
      sessionSkills: props.sessionSkills,
      onToggleSessionSkill: props.onToggleSessionSkill,
      devPipelines: props.devPipelines,
      onSelectDevPipeline: props.onSelectDevPipeline,
    ...
    [
      ...
      props.providerGroups,
      props.runtimeMode,
      props.pipelineTargetPolicy,
      props.onUpdatePipelineTargetPolicy,
      props.sessionSkills,
      props.onToggleSessionSkill,
      props.devPipelines,
      props.onSelectDevPipeline,
      searchQuery,
      showLegacyToggle,
      toggleProvider,
      toggleFavorite,
...
function ThreadSettingsOptionsItem(props: {
  ...
  const bottomToolbarInset =
    Platform.OS === "ios" && NATIVE_MAIL_SEARCH_TOOLBAR_SUPPORTED
      ? NATIVE_MAIL_SEARCH_TOOLBAR_CONTENT_INSET
      : 0;
  const hasSessionSkillsRow = (session.sessionSkills?.length ?? 0) > 0;
  const hasDevPipelineRow = (session.devPipelines?.names.length ?? 0) > 0;
  const hasPipelineTargetRow = session.pipelineTargetPolicy !== undefined;
    ...
    <View style={{ paddingBottom: insets.bottom + bottomToolbarInset + 12 }}>
      ...
      >
        ...
        <Animated.View layout={THREAD_SETTINGS_OPTIONS_LAYOUT_TRANSITION}>
          <DisclosureRow
            isLast={!hasSessionSkillsRow && !hasDevPipelineRow && !hasPipelineTargetRow}
            label="Runtime"
            value={
        ...
        {hasSessionSkillsRow ? (
          <Animated.View layout={THREAD_SETTINGS_OPTIONS_LAYOUT_TRANSITION}>
            <DisclosureRow
              isLast={!hasDevPipelineRow && !hasPipelineTargetRow}
              label="Chat skills"
              value={`${session.sessionSkills?.filter((skill) => skill.enabled).length ?? 0} enabled`}
              onPress={() => props.onOpenSubmenu({ kind: "session-skills" })}
            />
          </Animated.View>
        ) : null}
        {hasDevPipelineRow ? (
          <Animated.View layout={THREAD_SETTINGS_OPTIONS_LAYOUT_TRANSITION}>
            <DisclosureRow
              isLast={!hasPipelineTargetRow}
              label="Dev pipeline"
              value={
                session.devPipelines?.supported
                  ? (session.devPipelines.activeName ?? "Off")
                  : "Unavailable"
              }
              onPress={() => props.onOpenSubmenu({ kind: "dev-pipeline" })}
            />
          </Animated.View>
        ) : null}
        {hasPipelineTargetRow ? (
          <Animated.View layout={THREAD_SETTINGS_OPTIONS_LAYOUT_TRANSITION}>
            <DisclosureRow
              isLast
              label="Pipeline targets"
              value={session.pipelineTargetPolicy === "exact" ? "Exact only" : "Labeled fallback"}
              onPress={() => props.onOpenSubmenu({ kind: "pipeline-target-policy" })}
            />
          </Animated.View>
        ) : null}
      </Animated.View>
...
function ThreadSettingsChoiceContent(props: {
  ...
  const submenuContent =
    props.submenu.kind === "runtime"
      ...
      : props.submenu.kind === "session-skills"
        ? {
            rows: (session.sessionSkills ?? []).map((skill) => ({
              id: skill.name,
              ...
              onPress: () => {
                void Haptics.selectionAsync();
                session.onToggleSessionSkill?.(skill.name);
              },
            })),
          }
        : props.submenu.kind === "dev-pipeline"
          ? {
              rows: [
                {
                  id: "off",
                  label: "Off",
                  description: undefined,
                  selected: session.devPipelines?.activeName === null,
                  disabled: false,
                  onPress: () => {
                    void Haptics.selectionAsync();
                    session.onSelectDevPipeline?.(null);
                    props.onSelected();
                  },
                },
                ...(session.devPipelines?.names ?? []).map((name) => ({
                  id: name,
                  label: name,
                  description: undefined,
                  selected: session.devPipelines?.activeName === name,
                  disabled: !session.devPipelines?.supported,
                  onPress: () => {
                    void Haptics.selectionAsync();
                    session.onSelectDevPipeline?.(name);
                    props.onSelected();
                  },
                })),
              ],
            }
          : props.submenu.kind === "pipeline-target-policy"
            ? {
                rows: [
                  {
                    id: "labeled-fallback",
                    ...
                    description: undefined,
                    selected: session.pipelineTargetPolicy === "labeled-fallback",
                    disabled: false,
                    onPress: () => {
                      void Haptics.selectionAsync();
                      session.onUpdatePipelineTargetPolicy?.("labeled-fallback");
                      props.onSelected();
                    },
                  },
                  {
                    ...
                    description: undefined,
                    selected: session.pipelineTargetPolicy === "exact",
                    disabled: false,
                    onPress: () => {
                      void Haptics.selectionAsync();
                      session.onUpdatePipelineTargetPolicy?.("exact");
                      props.onSelected();
                    },
                  },
                ],
          ...
          onOpenSubmenu={(submenu) => {
            const title =
              submenu.kind === "runtime"
                ? "Runtime"
                : submenu.kind === "session-skills"
                  ? "Chat skills"
                  : submenu.kind === "dev-pipeline"
                    ? "Dev pipeline"
                    : submenu.kind === "pipeline-target-policy"
                      ? "Pipeline targets"
                      : (session.displayedDescriptors.find(
                          (descriptor) =>
                            descriptor.type === "select" && descriptor.id === submenu.id,
                        )?.label ?? "Option");
