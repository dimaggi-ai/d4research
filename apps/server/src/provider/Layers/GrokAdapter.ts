...
import { stableStringify } from "@d4research/shared/relaySigning";
...
export interface GrokAdapterLiveOptions {
  ...
  readonly providerIdentity?: { readonly kind: ProviderDriverKind; readonly displayName: string };
...
export function makeGrokAdapter(grokSettings: GrokSettings, options?: GrokAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const PROVIDER = options?.providerIdentity?.kind ?? ProviderDriverKind.make("grok");
    const providerDisplayName = options?.providerIdentity?.displayName ?? "Grok";
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
              ...
              Effect.gen(function* () {
                ...
                switch (event._tag) {
                  ...
                  case "AssistantItemCompleted":
                    if (event.streamKind !== "reasoning_text" && event.text !== undefined) {
          ...
          yield* acp.exitCode.pipe(
            Effect.flatMap((exitCode) =>
              withThreadLock(
                ...
                Effect.gen(function* () {
                  ...
                  yield* settlePendingUserInputsAsCancelled(ctx.pendingUserInputs);
                  sessions.delete(ctx.threadId);
                  const reason = `${providerDisplayName} ACP process exited unexpectedly with code ${String(exitCode)}.`;
                  yield* offerRuntimeEvent({
                    type: "session.exited",
                    ...(yield* makeEventStamp()),
                    provider: PROVIDER,
            ...
            Effect.catchCause((cause) =>
              Effect.logError(`Failed to reconcile an exited ${providerDisplayName} ACP process.`, {
                cause,
              }),
            ),
