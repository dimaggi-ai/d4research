...
import { type DeepPartial, deepMerge } from "@d4research/shared/Struct";
import { fromJsonStringPretty, fromLenientJson } from "@d4research/shared/schemaJson";
import {
  applyServerSettingsPatch,
  deriveLegacyProjectOverrides,
  isModelSelectionProviderEnabled,
} from "@d4research/shared/serverSettings";
import * as ServerSecretStore from "./auth/ServerSecretStore.ts";

export { resolveSourceControlWriterModelSelection } from "@d4research/shared/serverSettings";
...
>()("d4research/serverSettings/ServerSettingsService") {
...
const make = Effect.gen(function* () {
  ...
  const loadSettingsFromDisk = Effect.gen(function* () {
    ...
    const providerHistory = yield* sql<{
      ...
      WHERE provider_name IN ('cursor', 'grok', 'opencode', 'muse')
      UNION
      SELECT DISTINCT
        provider_name AS "providerName",
        provider_instance_id AS "providerInstanceId"
