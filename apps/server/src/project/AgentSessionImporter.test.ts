  ...
  for (const source of ["codex", "claudeAgent"] as const) {
    it.effect(`resumes imported ${source} history only after the first prompt`, () =>
      Effect.gen(function* () {
        ...
        const turnSent = yield* Deferred.make<void, Error>();
        ...
        const reactorLayer = ProviderCommandReactorLive.pipe(
          Layer.provide(
            Layer.succeed(InlineDelegationRunner, {
              run: () => Effect.die("unexpected delegation"),
            }),
          ),
            ...
            makeProviderRegistryLayer([
              {
                ...
                checkedAt: "2026-08-24T10:02:00.000Z",
        ...
        yield* Effect.gen(function* () {
          ...
          const startupEvents = engine.subscribeDomainEvents
            ? yield* engine.subscribeDomainEvents
            : engine.streamDomainEvents;
          yield* startupEvents.pipe(
            Stream.runForEach((event) =>
              ...
              event.payload.activity.tone === "error"
                ? Deferred.fail(
                    turnSent,
                    new Error(
                      `Imported session resume failed: ${event.payload.activity.summary} ${encodeTranscriptRecord(event.payload.activity.payload)}`,
                    ),
                  )
                : Effect.void,
            ),
