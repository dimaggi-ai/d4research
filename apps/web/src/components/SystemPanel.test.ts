...
describe("SystemPanel", () => {
  ...
  it("keeps token and provider usage out of the environment-health surface", () => {
    const markup = renderToStaticMarkup(createElement(SystemPanel));

    expect(markup).toContain("Tool Guard");
    expect(markup).not.toContain("Token Usage");
    expect(markup).not.toContain("Per-turn token usage");
