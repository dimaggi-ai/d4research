...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  if (action === "download") {
    return "This Mac has Apple Silicon, but T3Research is still running the Intel build under Rosetta. Download the available update to switch to the native Apple Silicon build.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but T3Research is still running the Intel build under Rosetta. Restart to install the downloaded Apple Silicon build.";
  }
  return "This Mac has Apple Silicon, but T3Research is still running the Intel build under Rosetta. The next app update will replace it with the native Apple Silicon build.";
}
...
export function getDesktopUpdateInstallConfirmationMessage(
  ...
  const windowsInstallWarning = isWindowsPlatform(platform)
    ? "\n\nOn Windows, T3Research may remain closed for several minutes while the update installs, and no installer window may appear. T3Research will reopen automatically when installation finishes."
    : "";
  return `Install update${version ? ` ${version}` : ""} and restart T3Research?\n\nAny running tasks will be interrupted. Make sure you're ready before continuing.${windowsInstallWarning}`;
}
