...
} from "@d4research/mobile-markdown-text/markdown";
...
describe("nativeMarkdownDocumentRuns", () => {
  it("renders a file mention without swallowing sentence punctuation or changing package references", () => {
      ...
      children: [
        {
          type: "paragraph",
          children: [
            { type: "text", content: "Inspect @src/Checkout.tsx. Use @d4research/contracts." },
          ],
        },
      ],
    });
    expect(runs).toEqual([
      ...
      { text: ". Use @d4research/contracts.", role: "body" },
    ]);
  });
...
describe("composerChipSizeSuffix", () => {
  it("labels attachment records with a human size, matching web's chip", async () => {
    const { composerChipSizeSuffix } = await import("@d4research/mobile-markdown-text/markdown");
...
describe("contextChipPresentation image detection", () => {
  it("treats a picture attached through the file picker as an image", async () => {
    const { contextChipPresentation } = await import("@d4research/mobile-markdown-text/markdown");
