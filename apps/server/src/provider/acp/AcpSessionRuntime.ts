...
export interface AcpSessionRuntimeOptions {
  ...
  /** Compatibility option for agents that require small JSON-RPC request ids. */
...
>()("d4research/provider/acp/AcpSessionRuntime") {}
...
interface AcpAssistantSegmentState {
  ...
  readonly activeItem?: { readonly itemId: string; readonly text: string };
}
    ...
    for (const event of parsed.events) {
      ...
      if (event._tag === "ContentDelta") {
        if (event.text.trim().length === 0) {
          ...
          if (!assistantSegmentState.activeItem) {
            continue;
          }
        }
        ...
        yield* Ref.update(assistantSegmentRef, (current) =>
          current.activeItem?.itemId === itemId
            ? {
                ...current,
                activeItem: {
                  ...current.activeItem,
                  text: `${current.activeItem.text}${event.text}`,
                },
              }
            : current,
        );
  ...
  Ref.modify<AcpAssistantSegmentState, EnsureActiveAssistantSegmentResult>(
    ...
    (current) => {
      if (current.activeItem) {
        return [{ itemId: current.activeItem.itemId }, current] as const;
      }
    ...
    return [
      {
        ...
        itemId: current.activeItem.itemId,
        streamKind: "assistant_text",
        ...(current.activeItem.text.length > 0 ? { text: current.activeItem.text } : {}),
