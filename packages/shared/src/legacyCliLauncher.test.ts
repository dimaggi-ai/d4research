...
it.skipIf(hostPlatform === "win32")(
  ...
  async () => {
    const root = await NodeFSP.mkdtemp(
      NodePath.join(NodeOS.tmpdir(), "d4research-legacy-launcher-"),
    );
    const entry = NodePath.join(root, "node_modules/d4research/dist/bin.mjs");
    const executable = NodePath.join(
      root,
      `node_modules/@d4research/d4research-${hostPlatform}-${hostArch}/d4research`,
    );
