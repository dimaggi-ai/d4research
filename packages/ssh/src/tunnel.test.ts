...
describe("ssh tunnel scripts", () => {
  ...
  it("shell-quotes package specs in the remote t3 runner", () => {
    ...
    assert.include(
      script,
      "exec npm exec --yes 'd4research@nightly; touch /tmp/t3-owned' -- \"$@\"",
    );
  ...
  it("uses the remote t3 runner for launch and pairing scripts", () => {
    ...
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "d4research@nightly" }),
      "d4research@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "d4research@nightly" }),
      "d4research@nightly",
    );
    assert.include(
