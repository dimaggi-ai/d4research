...
- `apps/web` (`@d4research/web`): React + Vite UI. Consumes the shared client runtime and adds routing,
  components, and web-specific platform layers.
- `apps/desktop` (`@d4research/desktop`): Electron shell. Supervises a desktop-scoped `t3` backend,
  loads the web bundle over the `t3code://` protocol, and owns SSH-managed remote environments.
- `apps/mobile` (`@d4research/mobile`): Expo/React Native client. Same client runtime composition as
  web, different platform layer and UI.
- `apps/marketing` (`@d4research/marketing`): Astro marketing site.

## packages

- `packages/contracts` (`@d4research/contracts`): shared Effect Schema definitions. RPC group,
  orchestration commands/events/read model, auth scopes, environment descriptors, settings.
- `packages/shared` (`@d4research/shared`): framework-agnostic utilities used by server and clients
  (`DrainableWorker`, git and source-control helpers, relay auth and signing, DPoP, semver, logging,
  observability, and more).
- `packages/client-runtime` (`@d4research/client-runtime`): connection lifecycle, authorization, RPC
...
- `packages/ssh` (`@d4research/ssh`): SSH config parsing, auth prompts, command execution, and the
  tunnel/environment manager behind desktop-managed SSH environments.
- `packages/tailscale` (`@d4research/tailscale`): Tailscale CLI wrapper, including the
...
`@d4research/shared` and `@d4research/client-runtime` use explicit subpath exports with no barrel index and
no root export. Import the narrow path (`@d4research/shared/DrainableWorker`,
`@d4research/client-runtime/state/threads`) rather than the package root. Files that are not exported
are implementation details. `@d4research/contracts` does export a root alongside `./settings` and
`./relay`.
