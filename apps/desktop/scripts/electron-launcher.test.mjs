...
describe("electron development launcher", () => {
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/d4research (Dev).app",
      "d4research (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "d4research (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/d4research (Dev).app/Contents/MacOS/d4research (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/d4research (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/d4research (Dev).app/Contents/MacOS/Electron'",
    );
    assert.notInclude(script, "node_modules/electron");
  });

  it("declares why the macOS app needs protected access", () => {
    const values = resolveMacBundleInfoPlistStrings("d4research (Dev) Launcher");

    assert.equal(
      values.NSScreenCaptureUsageDescription,
      "d4research captures the active window when you use the snapshot shortcut.",
    );
    assert.equal(
      values.NSDocumentsFolderUsageDescription,
      "d4research reads project files you open in the desktop app.",
    );
  });

  it("ad-hoc signs the complete development app bundle", () => {
    assert.deepEqual(resolveMacCodeSignArguments("/runtime/d4research (Dev).app"), [
      "--force",
      "--deep",
      "--sign",
      "-",
      "--timestamp=none",
      "/runtime/d4research (Dev).app",
    ]);
  });
