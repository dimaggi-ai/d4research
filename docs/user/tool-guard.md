...
Tool Guard is an optional environment-local integration for macOS and Linux. It adds a shared policy
layer and audit log on the machine running the d2research environment. Installing it on one
environment does not change other local or remote environments. Managed installation on Windows is
not yet available.
