...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/d4research (Nightly).app/Contents/MacOS/d4research",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/d4research.app/Contents/MacOS/d4research")).toBe(
      "/Applications/d4research.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/d4research.app/other/MacOS/d4research")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  await open();
  const helperPage = decodeURIComponent(String(mocks.loadURL.mock.calls[0]?.[0]));
  expect(helperPage).toContain("Drag d4research into the list above");
  expect(helperPage).not.toContain("T3 Code");
  send("drag", false);
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/d4research (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/d4research (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged d4research icon is missing");
  expect(windows).toHaveLength(0);
});
