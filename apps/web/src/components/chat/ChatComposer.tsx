import { type ServerProviderSkill, type TurnId } from "@d4research/contracts";
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const composerControls = showProviderUnavailable ? (
    ...
    <>
      ...
      <span
        ...
        data-resting-fixed-control={composerControlsInStrip ? "true" : undefined}
      >
  ...
  const onComposerPaste = (event: React.ClipboardEvent<HTMLElement>) => {
    ...
    if (textFiles.length > 0) {
      ...
      const remaining = files.filter((file) => !isTextLikeFile(file));
      if (remaining.length > 0) void addComposerAttachments(remaining);
      return;
    }
