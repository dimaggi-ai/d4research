...
`agent-device` command line, which d4research sets up for them.
...
Studio's Device Manager. d4research detects standard SDK locations; set
...
timeline. Agents drive the device through the `agent-device` command line. d4research
installs and starts it only after **Agent device access** is enabled.
Restart an existing agent session after granting access so it
...
d4research checks common Homebrew and Android SDK locations; custom installations need
...
sessions and stops its d4research helpers when reachable; simulators keep running.

d4research provides discovery, streaming, and control. Arrange app builds,
