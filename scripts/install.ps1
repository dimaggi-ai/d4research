...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    try {
      ...
      if ($status -eq 404) {
        Fail "t3 $version has no release archive for win32-$arch; releases before the self-contained CLI can only be installed with 'npm install -g d4research@$version'"
      }
      throw
    }
