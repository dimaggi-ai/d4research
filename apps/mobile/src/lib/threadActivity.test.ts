...
it("preserves the resolved d4 delegate model on feed activities", () => {
  const thread = makeThread({
    id: ThreadId.make("delegate-thread"),
    projectId: ProjectId.make("delegate-project"),
    title: "Delegation",
    activities: [
      makeActivity({
        ...
        payload: {
          ...
          status: "completed",
          data: { researchDelegate: { target: "codex:gpt-5.6-sol", failed: false } },
        },
      }),
    ],
  });
  expect(buildThreadFeed(thread)).toMatchObject([
    { type: "activity-group", activities: [{ researchDelegateTarget: "codex:gpt-5.6-sol" }] },
  ]);
});
