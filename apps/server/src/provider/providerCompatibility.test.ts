...
const policy: ProviderCompatibilityPolicy = {
  driver,
  // d4research: the server package is 0.2.x, so the window must include it.
...
describe("provider compatibility", () => {
  it("bundles a compatibility policy for every built-in harness", () => {
    for (const builtIn of BUILT_IN_DRIVERS) {
      // d4research: fork-only drivers have no upstream compatibility policy.
      if (["agy", "muse", "junie"].includes(builtIn.driverKind)) continue;
      assert.isDefined(
        resolveProviderCompatibility(
