  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "slash-command") {
      const builtInSlashCommandItems = [
        ...
        },
        {
          ...
          command: "compact",
          label: "/compact",
          description: "Compact this agent's context while keeping the chat history",
        },
      ...
      const providerSlashCommandItems = (selectedProviderStatus?.slashCommands ?? [])
        .filter((command) => !builtInCommandNames.has(command.name.toLowerCase()))
        .map((command) => ({
    ...
    if (composerTrigger.kind === "skill") {
      return searchProviderSkills(composerSkills, composerTrigger.query).map((skill) => ({
        ...
        description: providerReportsSkills
          ? (skill.shortDescription ??
            skill.description ??
            (skill.scope ? `${skill.scope} skill` : "Run provider skill"))
          : describeComposerFallbackSkill(skill),
      }));
    }
    return [];
  }, [
