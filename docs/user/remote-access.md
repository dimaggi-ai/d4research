...
Run the command from the d4research checkout. It finds that checkout's running development server,
issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your
phone. Use `--base-dir` when the server uses an explicit T3 home.
...
run a headless server from the CLI, or have the desktop app launch d4research over SSH.
...
Run the source CLI from the checkout.
...
Use this when you want the desktop app to start or reuse d4research on another machine over SSH.
...
The remote host must have a compatible Node.js runtime. d4research uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, d4research first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path d4research uses:
...
When the d4research web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: d4research may
...
see [Keeping d4research in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running d4research in the Background](./background-service.md).
