...
- The full dev stack: `vp run dev`, then open the pairing URL it prints through the forwarded web port. The main checkout defaults to 5733; worktrees and occupied ports can select another port, so use the `[dev-runner]` output and forward that port. The bare origin is useless without the pairing token. In VS Code the forwarded port is a true localhost, so the printed URL works as-is; in browser Codespaces the forwarded origin differs, and if the server rejects it, pass the forwarded origin via `T3CODE_DEV_ALLOWED_ORIGINS`.
