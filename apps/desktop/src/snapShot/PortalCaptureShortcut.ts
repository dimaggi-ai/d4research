...
import type { SnapShotKeyChord } from "@d4research/contracts";
...
export class PortalCaptureShortcut {
  ...
  async configure() {
    ...
    if (!this.hasSession || this.version < 2)
      throw new Error(
        "Open your desktop's shortcut settings and allow d4research's capture shortcut.",
      );
    await this.call({
  ...
  private failed = (error: unknown) => {
    ...
    this.update({
      ...
      shortcutMessage: this.managedByHyprland
        ? "Couldn't connect to Hyprland shortcuts. Make sure xdg-desktop-portal-hyprland is running, then restart d4research."
          ...
          this.update({
            ...
            shortcutMessage:
              this.version >= 2
                ...
                : "Shortcut permission wasn't granted. Allow d4research in your desktop's shortcut settings.",
          });
          return undefined;
        }
    ...
    if (this.managedByHyprland) {
      this.update({
        ...
        shortcutMessage: shortcut
          ...
          : "Hyprland did not register the capture action. Check that xdg-desktop-portal-hyprland is running, then restart d4research.",
      });
      return;
    }
