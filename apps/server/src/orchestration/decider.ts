...
> {
  ...
  const leadingEvents: PlannedOrchestrationEvent[] = [];
  if (thread.settledOverride !== null) {
    leadingEvents.push({
      ...
      payload: { threadId: thread.id, reason: "activity", updatedAt: occurredAt },
    });
  }
  ...
  if (
    ...
    (message.modelSelection.instanceId !== thread.modelSelection.instanceId ||
      message.modelSelection.model !== thread.modelSelection.model)
  ) {
    leadingEvents.push({
      ...(yield* withEventBase({
        aggregateKind: "thread",
  ...
  return [...leadingEvents, userMessageEvent, turnStartRequestedEvent];
});
