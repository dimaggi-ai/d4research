/**
 * `dist/bin.mjs` of the `d4research` npm package: the entry point boot-service
 ...
 * executable in the sibling `@d4research/d4research-<platform>-<arch>` package.
 *
...
const executableName = process.platform === "win32" ? "d4research.exe" : "d4research";
const executable = join(dirname(require.resolve("@d4research/d4research-" + process.platform + "-" + process.arch + "/package.json")), executableName);
...
const fail = (error) => {
  if (!error) return;
  process.stderr.write("d4research: " + error.message + "\\n");
  child.kill("SIGTERM");
  process.exitCode = 1;
};
if (ipc) {
