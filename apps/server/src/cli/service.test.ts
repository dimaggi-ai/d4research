...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "T3 Code service",
      "  Status: installed · d4research@0.0.29",
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx d4research@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx d4research@0.0.32-nightly.1 service update");
  expect(output).not.toContain("d4research@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx d4research@0.0.29 service update");
  expect(output).not.toContain("npx d4research@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "d4research@0.0.32-nightly.1 (newer than this d4research@0.0.31 CLI)");
  assert.include(output, "npx d4research@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx d4research@latest service update");
});
