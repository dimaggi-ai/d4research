...
import { PROJECT_FAVICON_MAX_DATA_URL_LENGTH } from "@d4research/client-runtime/project-favicon-cache";
...
const native = vi.hoisted(() => ({
  ...
  write: vi.fn(async () => ({ uri: "file:///cache/thumbnail" })),
  releaseRendered: vi.fn(),
...
vi.mock("expo-image-manipulator", () => ({
  ...
  ImageManipulator: {
    manipulate: () => ({
      renderAsync: async () => ({ saveAsync: native.write, release: native.releaseRendered }),
...
describe("mobile project icon thumbnails", () => {
  it("reduces an oversized encoding and deletes temporary thumbnail files", async () => {
    ...
    expect(native.releaseRendered).toHaveBeenCalledTimes(2);
