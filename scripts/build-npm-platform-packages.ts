 ...
 * behind `npx d4research` / `npm i -g d4research`: one
 * `@d4research/d4research-<platformKey>` package per
 * archive holding the archive's contents verbatim, plus the `d4research` launcher
 ...
 *
 *   @d4research/d4research-<platformKey>/      archive contents flattened + package.json
 *   @d4research/d4research-<platformKey>.tgz   the same tree as an npm tarball
 *   d4research/                                launcher: package.json, bin/d4research.js, README.md
 *   d4research.tgz                             the launcher as an npm tarball
 *
...
export const NPM_PLATFORM_PACKAGE_SCOPE = "@d4research";
export const NPM_LAUNCHER_PACKAGE_NAME = "d4research";
const NPM_EXECUTABLE_NAME = "d4research";
...
export function npmPlatformPackageName(platformKey: CliArchivePlatformKey): string {
  return `${NPM_PLATFORM_PACKAGE_SCOPE}/d4research-${platformKey}`;
}
...
export function npmPlatformPackageManifest(
  ...
  return {
    ...
    version,
    description: `d4research CLI executable for ${platformKey}`,
...
export function npmPlatformPackageReadme(platformKey: CliArchivePlatformKey): string {
  return [
    ...
    "",
    `The d4research CLI executable for ${platformKey}. Do not install this package directly:`,
...
/** package.json for the `d4research` launcher. No engines: bin/d4research.js is trivial CJS. */
export function npmLauncherPackageManifest(
  ...
  return {
    ...
    version,
    description: "d4research CLI. Installs the self-contained executable for this platform.",
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    bin: { [NPM_LAUNCHER_PACKAGE_NAME]: `./bin/${NPM_LAUNCHER_PACKAGE_NAME}.js` },
    files: ["bin", "dist"],
...
let packageDir;
try {
  packageDir = dirname(require.resolve("${NPM_PLATFORM_PACKAGE_SCOPE}/d4research-" + key + "/package.json"));
} catch {
  process.stderr.write(
    [
      "d4research: no d4research CLI build is available for this platform (" + key + ").",
      "Supported platforms: " + SUPPORTED.join(", ") + ".",
      "If yours is listed, reinstall d4research so npm fetches its optional dependency.",
...
const executable = join(packageDir, process.platform === "win32" ? "d4research.exe" : "d4research");
const result = spawnSync(executable, process.argv.slice(2), { stdio: "inherit" });
if (result.error) {
  process.stderr.write("d4research: failed to start " + executable + ": " + result.error.message + "\\n");
  process.exit(1);
}
...
const stagePlatformPackage = Effect.fn("stagePlatformPackage")(function* (input: {
  ...
  const executableName = input.key.startsWith("win32")
    ? `${NPM_EXECUTABLE_NAME}.exe`
    : NPM_EXECUTABLE_NAME;
  ...
  if (executableName === NPM_EXECUTABLE_NAME) {
...
/** Writes the launcher package (package.json, bin/d4research.js, README) and its tarball. */
const stageLauncherPackage = Effect.fn("stageLauncherPackage")(function* (input: {
  ...
  const launcherScript = path.join(stageDir, `bin/${NPM_LAUNCHER_PACKAGE_NAME}.js`);
...
const command = Command.make(
  "build-npm-platform-packages",
  {
    archivesDir: Flag.String("archives-dir").pipe(
      Flag.withDescription(
        "Directory holding the release's d4research-<version>-<platform> archives.",
      ),
    ),
  ...
  Command.withDescription(
    "Build the d4research launcher and @d4research/d4research-<platform> npm packages from CLI release archives.",
  ),
);
