 ...
 * archive: the `d4research` binary, the web client, the resource monitor, and a
 ...
 * archive stem):
 *
 *   d4research | d4research.exe          the single-executable
...
export function cliArchiveStem(version: string, platform: BuildPlatform, arch: BuildArch): string {
  return `d4research-${version}-${cliArchivePlatformKey(platform, arch)}`;
}
...
const signMacArchiveContents = Effect.fn("signMacArchiveContents")(function* (input: {
  ...
  const notarizeZip = path.join(path.dirname(input.executablePath), ".notarize-d4research.zip");
  yield* runCommand(
  ...
  yield* Effect.log("[cli-archive] Notarized d4research.");
});
...
/** Signs d4research.exe through the same Azure Trusted Signing setup the installer uses. */
const signWindowsExecutable = Effect.fn("signWindowsExecutable")(function* (
  ...
  yield* runCommand(
    ...
    "Invoke-TrustedSigning d4research.exe",
  );
  yield* Effect.log("[cli-archive] Signed d4research.exe (Azure Trusted Signing).");
});
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  const executableName = input.platform === "win" ? "d4research.exe" : "d4research";
  // tsdown suffixes cross-built executables with their target (d4research-darwin-x64);
  // a host build is plain d4research. Prefer the exact target when both exist.
  const targetKey = `${input.platform === "mac" ? "darwin" : input.platform}-${input.arch}`;
  const targetExecutable = path.join(
    serverDir,
    "dist-exe",
    `d4research-${targetKey}${input.platform === "win" ? ".exe" : ""}`,
  );
  ...
  yield* requireInput(
    ...
    "Run `vp run --filter d4research build` first.",
  );
  yield* requireInput(
    resourceMonitorDir,
  ...
  const stageRoot = yield* fs.makeTempDirectoryScoped({ prefix: "d4research-cli-archive-" });
...
const command = Command.make(
  ...
  Command.withDescription("Package the d4research single-executable into a per-platform archive."),
);
