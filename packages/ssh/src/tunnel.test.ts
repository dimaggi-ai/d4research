...
describe("ssh tunnel scripts", () => {
  it("builds the remote t3 runner with npx and npm fallbacks", () => {
    ...
    assert.include(script, "exec npx --yes 'd4research@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'd4research@latest' -- \"$@\"");
    assert.include(script, "could not install 'd4research@latest'");
    assert.include(script, "require_installed_t3_cli npx --yes --package 'd4research@latest'");
    assert.include(script, "require_installed_t3_cli npm exec --yes --package 'd4research@latest'");
  ...
  it("shell-quotes package specs in the remote t3 runner", () => {
    const script = buildRemoteT3RunnerScript({
      packageSpec: "d4research@nightly; touch /tmp/t3-owned",
    });

    assert.include(script, "exec npx --yes 'd4research@nightly; touch /tmp/t3-owned' \"$@\"");
    assert.include(script, "exec npm exec --yes 'd4research@nightly; touch /tmp/t3-owned' -- \"$@\"");
    assert.include(
      script,
      "require_installed_t3_cli npx --yes --package 'd4research@nightly; touch /tmp/t3-owned'",
    );
    assert.notInclude(script, "exec npx --yes d4research@nightly; touch /tmp/t3-owned");
  });
  ...
  it("uses the remote t3 runner for launch and pairing scripts", () => {
    ...
    assert.include(buildRemoteLaunchScript({ packageSpec: "d4research@nightly" }), "d4research@nightly");
    assert.include(
    ...
    assert.include(buildRemotePairingScript(target, { packageSpec: "d4research@nightly" }), "d4research@nightly");
    assert.include(
