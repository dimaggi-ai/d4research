...
d4research uses each coding provider's native permissions by default. The access selector beside the
...
layer and audit log on the machine running the d4research environment. Installing it on one
...
organization. Install it on the machine running the d4research environment, not on a phone or remote
...
`T3RESEARCH_TOOL_GUARD_BIN` to its absolute path before starting d4research.

## Manage the d4research integration

1. Open **Settings → General → Agent permissions**.
2. Under **d4research Tool Guard**, select **Install**.
...
external Tool Guard hook already exists, d4research lists the provider configuration files that own
it and offers **Replace with d4research**. Replacement requires confirmation, removes only hook
...
The d4research installation copies the detected Core executable, provider wrappers, and policy profiles
into the environment's d4research data directory. Provider configuration files retain unrelated
hooks when the managed integration is disabled or removed.

On Windows, d4research detects `tg.exe`, installs PowerShell hook adapters, and invokes them with a
