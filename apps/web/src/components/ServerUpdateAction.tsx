      ...
      if (desktopTargets.length > 0) {
        const confirmed =
          (await requestConfirmDialog(
            `Update the d4research desktop apps on ${desktopTargets.map((target) => target.serverLabel).join(", ")}? They will close and relaunch on those machines.`,
          )) ?? true;
        if (!confirmed) return;
      }
      await Promise.all(
...
export function ServerUpdateAction({
  ...
  const handleUpdate = async () => {
    ...
    if (isDesktopAppUpdate) {
      ...
      const confirmed =
        (await requestConfirmDialog(
          `Update the d4research desktop app that runs the ${serverLabel}? It will close and relaunch on that machine.`,
        )) ?? true;
      if (!confirmed) {
        return;
      }
