...
import type { DesktopCaptureExtensionState } from "@d4research/contracts";
...
export async function installGnomeCaptureBundle({ bundle, dataHome }: SetupPaths) {
  ...
  if (existing) {
    ...
    if (installed.version > metadata.version)
      throw new Error("A newer extension is installed. Update d4research instead of replacing it.");
  }
    ...
    try {
      ...
      if (installed && (!info.state || (info.version && installed.version > info.version.value)))
        return {
          ...
          message:
            "Installed. Save your work, sign out of GNOME and sign back in, then return here to enable the extension. Restarting d4research alone is not enough.",
        };
      ...
      if (!properties.UserExtensionsEnabled.value)
        return {
          ...
          message:
            "GNOME has disabled user extensions. Turn on Extensions in the GNOME Extensions app, then check again. d4research will not enable your other extensions for you.",
        };
      if (info.state?.value === 1)
        return {
          status: "enabled",
          message: "The d4research extension is running. Active-window snapshots are available.",
        };
      ...
      return {
        status: "disabled",
        message:
          "Enable the d4research extension to allow active-window snapshots. You can disable it here at any time.",
      };
    } catch (error) {
      return {
        status: "error",
