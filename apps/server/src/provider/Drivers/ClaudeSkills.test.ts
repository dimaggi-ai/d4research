import { skillOverrideSettingsPaths, collectSkillDirectories } from "./ClaudeSkills.ts";
...
it.layer(NodeServices.layer)("discoverClaudeSkills", (it) => {
  ...
  it.effect("prefers user skills over project skills to match Claude CLI resolution", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-claude-skills-" });
  ...
  it.effect("collects inventory skills nested in category subdirectories", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-claude-skills-" });
      ...
      const directories = yield* collectSkillDirectories(skillsDir);
      assert.deepEqual(directories.map((directory) => path.basename(directory)).sort(), [
        "copywriting",
        "depth-three",
        "system-doctor",
        "top-level",
      ]);
      assert.ok(directories.includes(path.join(skillsDir, "writing", "copywriting")));
    }),
  );
