...
import { BUILT_IN_THEME_IDS, type BuiltInThemeId } from "@d4research/shared/themePalettes";
...
if (import.meta.main) {
  ...
  for (const [filename, contents] of getGeneratedUniwindThemeOutputs()) {
    if (checkOnly) {
      ...
      if (current !== contents) {
        console.error(
          `${NodePath.relative(process.cwd(), filename)} is stale. Run vp run --filter @d4research/mobile generate.`,
        );
        process.exitCode = 1;
      }
      continue;
