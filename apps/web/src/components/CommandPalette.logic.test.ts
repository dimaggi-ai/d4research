...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@d4research/contracts";
...
describe("buildThreadActionItems", () => {
  ...
  it("keeps message excerpts searchable without replacing thread metadata", () => {
    const [item] = buildThreadActionItems({
      ...
      projectTitleById: new Map([[PROJECT_ID, "d4research"]]),
      sortOrder: "updated_at",
      icon: null,
      getContentMatch: () => ({
        source: "assistant",
    ...
    expect(item?.description).toBe("d4research · #feat/search");
  });
  ...
  it("prefers renderDescription when provided", () => {
    ...
    expect(item?.description).toBe("d4research:feat/search:wt");
  });
