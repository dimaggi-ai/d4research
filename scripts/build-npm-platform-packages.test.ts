...
/** A tar.gz laid out like build-cli-archive.ts writes, with a stub `d4research` that echoes its args. */
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "d4research-npm-packages-test-" });
  const archivesDir = path.join(root, "archives");
  yield* fs.makeDirectory(archivesDir);
  for (const key of KEYS) {
    const stem = `d4research-${VERSION}-${key}`;
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "d4research"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "d4research"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@d4research/d4research-darwin-arm64", "@d4research/d4research-linux-x64", "d4research"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@d4research/d4research-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@d4research/d4research-linux-x64");
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@d4research", "d4research", "d4research.tgz"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@d4research/d4research-linux-x64.tgz")],
      ...
      assert.isTrue(
        lines.some((line) => /^-rwxr-xr-x .* package\/d4research$/.test(line)),
        listing.stdout,
      );
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        const passthrough = yield* run(
          process.execPath,
          ["bin/d4research.js", "serve", "--port", "1234"],
          {
            cwd: launcherDir,
            env,
          },
        ...
        const unpack = yield* run(
          "tar",
          ["-xf", path.join(fixture.outputDir, "d4research.tgz"), "-C", installedLauncher],
          {
            cwd: fixture.root,
          },
        );
      ...
      const unsupported = yield* run(process.execPath, ["bin/d4research.js", "--version"], {
        cwd: launcherDir,
      ...
      assert.include(unsupported.stderr, "https://github.com/dimaggi-ai/d4research/releases");
    }),
  );
});
