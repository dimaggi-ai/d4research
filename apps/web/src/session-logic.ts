...
export { formatDuration } from "@d4research/shared/orchestrationTiming";
...
/**
 * Keep the plan sidebar on the plan that produced the running turn, even when
 * the user has navigated back to another thread while that turn is active.
 */
export function findSidebarProposedPlan(input: {
  ...
  if (!input.latestTurnSettled) {
    ...
    if (sourceProposedPlan) {
      ...
      if (sourcePlan) return toLatestProposedPlanState(sourcePlan);
    }
  }
...
function toDerivedWorkLogEntry(activity: OrchestrationThreadActivity): DerivedWorkLogEntry {
  ...
  if (!entry.detail && (activity.kind === "runtime.error" || activity.kind === "runtime.warning")) {
    ...
    if (message && message !== entry.label) entry.detail = message;
  }
