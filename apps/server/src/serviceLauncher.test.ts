...
// A pinned runtime is an executable at <versionDir>/d4research. The tests stand one up
...
const writeFakeRuntime = (
  ...
  Effect.gen(function* () {
    const entryPath = path.join(versionDir, "d4research");
...
it.layer(NodeServices.layer)("service state persistence", (it) => {
  ...
  it.effect("a fresh launcher clears a restart deferred by d4research update", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
